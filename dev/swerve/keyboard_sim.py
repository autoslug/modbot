...
def normalizeModules(moduleStates):
    ...
    if max_speed <= 1: return moduleStates
...
if __name__ == "__main__":
    ...
    modules = np.array([[np.cos(angle_1), np.sin(angle_1)], [np.cos(angle_2), np.sin(angle_2)], [np.cos(angle_3), np.sin(angle_3)]])*R
    ...
    while True:  # making a loop
        try:
            ...
            plot_vec([modules[:,0], modules[:,1]], origin=[[0,0,0],[0,0,0]])
            plot_vec([2*module_dirs[:,0], 2*module_dirs[:,1]], origin=[modules[:,0], modules[:,1]], color='r')
