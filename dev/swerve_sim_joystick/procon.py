...
class ProCon:
    ...
    def load_stick_calibration(self):
        ok, reply = self.spi_flash_read(
            ProCon.CALIBRATION_OFFSET, ProCon.CALIBRATION_LENGTH
        )
        if not ok:
    ...
    def send_subcommand(self, id, param, wait_for_reply=True):
        data = (
            (ProCon.OutputReportID.RUMBLE_SUBCOMMAND, self.subcommand_counter)
            + self.rumble_low
            + self.rumble_high
            + (id,)
            + param
    ...
    def send_rumble(self, low, high, duration):
        ...
        self.rumble_expire = (
            int(time.time() * 1000) + duration if (low or high) and duration else 0
        )
        data = (
            (ProCon.OutputReportID.RUMBLE, self.subcommand_counter)
            + self.rumble_low
            + self.rumble_high
        )
        self.subcommand_counter = (self.subcommand_counter + 1) & 0xFF
    ...
    def set_imu_sensitivity(self, sensitivity):
        return self.send_subcommand(
            ProCon.SubcommandID.SET_IMU_SENSITIVITY, sensitivity
        )

    def enable_vibration(self, enable):
        return self.send_subcommand(
            ProCon.SubcommandID.ENABLE_VIBRATION, (int(enable),)
        )
